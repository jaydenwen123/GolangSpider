...
> https://link.juejin.im?target=https%3A%2F%2Fnamc.in%2F2019-04-15-retries-timeouts-backoff
> )
> * 原文作者： [namc]( https://link.juejin.im?target=http%3A%2F%2Fnamc.in%2F )
> * 译文出自： [掘金翻译计划](
> https://link.juejin.im?target=https%3A%2F%2Fgithub.com%2Fxitu%2Fgold-miner
> )
> * 本文永久链接： [github.com/xitu/gold-m…](
> https://link.juejin.im?target=https%3A%2F%2Fgithub.com%2Fxitu%2Fgold-miner%2Fblob%2Fmaster%2FTODO1%2Fretries-timeouts-backoff.md
> )
> * 译者： [nettee](
...
* [退避]( #%E9%80%80%E9%81%BF )
* [分布式系统中的幂等性]( #%E5%88%86%E5%B8%83%E5%BC%8F%E7%B3%BB%E7%BB%9F%E4%B8%AD%E7%9A%84%E5%B9%82%E7%AD%89%E6%80%A7 )
...
例如：参考 ` connectTimeout` 和 ` socketTimeout` 的定义 [Mysql Connector 配置]( https://link.juejin.im?target=https%3A%2F%2Fdev.mysql.com%2Fdoc%2Fconnector-j%2F5.1%2Fen%2Fconnector-j-reference-configuration-properties.html )
...
答案取决于你的 API/Server 响应的实现细节。如果你有一个 REST API，请返回 [503 Service Unavailable]( https://link.juejin.im?target=https%3A%2F%2Ftools.ietf.org%2Fhtml%2Frfc7231%23section-6.6.1 ) ，而不是其他 5xx/4xx 错误码。这可以让客户端知道超时是由“临时的过载”引起的，而不是由于代码层面的错误。
...
` for (int attempts = 0; attempts < 5; attempts++) { try { DoWork(); break ; } catch { } Thread.Sleep(50); // 延迟 } 复制代码`

当使用一个连接至外部服务的库时，请检查它是否实现了重试策略，允许你配置重试的最大次数、重试之间的延迟等。

你还可以通过设置 [Retry-After]( https://link.juejin.im?target=https%3A%2F%2Ftools.ietf.org%2Fhtml%2Frfc7231%23section-7.1.3 ) 响应头，在服务器端实现重试的策略。
...
**1. [指数退避]( https://link.juejin.im?target=https%3A%2F%2Fen.wikipedia.org%2Fwiki%2FExponential_backoff )**
...
` int delay = 50 for (int attempts = 0; attempts < 5; attempts++) { try { DoWork(); break ; } catch { } Thread.sleep(delay); if (delay < MAX_DELAY) // MAX_DELAY 可能依赖于应用程序和业务逻辑 { delay *= 2; } } 复制代码`
...
**2. [带抖动的退避]( https://link.juejin.im?target=https%3A%2F%2Faws.amazon.com%2Fblogs%2Farchitecture%2Fexponential-backoff-and-jitter%2F )**
...
` int delay = 50 for (int attempts = 0; attempts < 5; attempts++) { try { DoWork(); break ; } catch { } Thread.sleep(delay); delay *= random.randrange(0, min(MAX_DELAY, delay * 2 ** i)) // 只是生成一个简单的随机数 } 复制代码`
...
**3. [断路器]( https://link.juejin.im?target=https%3A%2F%2Fen.wikipedia.org%2Fwiki%2FCircuit_breaker_design_pattern )**

断路器模式在处理服务的长时间瞬时故障时非常有用。它通过确定服务的可用性，防止客户端重试注定会失败的请求。

断路器设计模式要求在一系列的请求中保留连接的状态。让我们看看 [failsafe 实现的断路器]( https://link.juejin.im?target=https%3A%2F%2Fgithub.com%2Fjhalterman%2Ffailsafe%23circuit-breakers )

` CircuitBreaker breaker = new CircuitBreaker() .withFailureThreshold(5) .withSuccessThreshold(3) .withDelay(1, TimeUnit.MINUTES); Failsafe.with(breaker).run(() -> connect()); 复制代码`
...
* 对你的系统做性能统计，让它们时刻准备最坏的情况。你可以查看 [Netflix 的 Chaos Monkey]( https://link.juejin.im?target=https%3A%2F%2Fgithub.com%2FNetflix%2Fchaosmonkey ) —— 这是一个在系统中触发随机故障的弹性测试工具。这能让你为可能发生的故障做好准备，构建一个有弹性的系统。
* 如果你的系统由于某种原因处于过载状态，你可以尝试通过减载（load shedding）来分布负载。Google 做了一个很棒的 [案例研究]( https://link.juejin.im?target=https%3A%2F%2Fcloud.google.com%2Fblog%2Fproducts%2Fgcp%2Fusing-load-shedding-to-survive-a-success-disaster-cre-life-lessons ) ，可以作为一个很好的起点。

一些资源：

* [分布式系统中的模式]( https://link.juejin.im?target=https%3A%2F%2Fwww.dre.vanderbilt.edu%2F~schmidt%2Fpatterns-ace.html )
* [重试模式 - Microsoft]( https://link.juejin.im?target=https%3A%2F%2Fdocs.microsoft.com%2Fen-us%2Fazure%2Farchitecture%2Fpatterns%2Fretry )
* [Martin Fowler - 断路器]( https://link.juejin.im?target=https%3A%2F%2Fmartinfowler.com%2Fbliki%2FCircuitBreaker.html )
