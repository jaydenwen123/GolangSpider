...
前段时间写过一篇 [《线程池没你想的那么简单》]( https://link.juejin.im?target=https%3A%2F%2Fcrossoverjie.top%2F2019%2F05%2F20%2Fconcurrent%2Fthreadpool-01%2F ) ，和大家一起撸了一个基本的线程池，具备：
...
> https://link.juejin.im?target=https%3A%2F%2Fcrossoverjie.top%2F2019%2F05%2F20%2Fconcurrent%2Fthreadpool-01%2F
> )
> 
> 
...
![](https://user-gold-cdn.xitu.io/2019/6/6/16b2a490e651e59a?imageView2/0/w/1280/h/960/ignore-error/1)
...
![](https://user-gold-cdn.xitu.io/2019/6/6/16b2a490e64e9a8f?imageView2/0/w/1280/h/960/ignore-error/1)

为了简单的达到这个效果，我们可以在初始化线程池的时候传入一个接口的实现，这个接口就是用于任务完成之后的回调。

![](https://user-gold-cdn.xitu.io/2019/6/6/16b2a490e67a31b6?imageView2/0/w/1280/h/960/ignore-error/1)

` public interface Notify { /** * 回调 */ void notifyListen () ; } 复制代码`
...
![](https://user-gold-cdn.xitu.io/2019/6/6/16b2a490e689788c?imageView2/0/w/1280/h/960/ignore-error/1)

为了并发安全的考虑，这里的计数器采用了原子的 ` AtomicInteger` 。

![](https://user-gold-cdn.xitu.io/2019/6/6/16b2a490e6a0d64f?imageView2/0/w/1280/h/960/ignore-error/1)
...
` executorService.shutdown(); while (!executorService.awaitTermination( 100 , TimeUnit.MILLISECONDS)) { logger.info( "thread running" ); } 复制代码`
...
![](https://user-gold-cdn.xitu.io/2019/6/6/16b2a49122e2aa66?imageView2/0/w/1280/h/960/ignore-error/1)

这个接口有一个返回值。

同时在提交任务时也稍作改动：

![](https://user-gold-cdn.xitu.io/2019/6/6/16b2a49142e00293?imageView2/0/w/1280/h/960/ignore-error/1)

首先是执行任务的函数由 ` execute()` 换为了 ` submit()` ，同时他会返回一个返回值 ` Future` ，通过它便可拿到线程执行的结果。

最后通过第二步将所有执行结果打印出来：

![](https://user-gold-cdn.xitu.io/2019/6/6/16b2a4915941230e?imageView2/0/w/1280/h/960/ignore-error/1)
...
` public interface Callable < T > { /** * 执行任务 * @return 执行结果 */ T call () ; } 复制代码`
...
![](https://user-gold-cdn.xitu.io/2019/6/6/16b2a49180cadfea?imageView2/0/w/1280/h/960/ignore-error/1)

所以在它的 ` run()` 函数中会调用刚才提到的具有返回值的 ` call()` 函数。

再次结合 ` submit()` 提交任务和 ` get()` 获取返回值的源码来看会更加理解这其中的门道。

` /** * 有返回值 * * @param callable * @param <T> * @return */ public <T> Future<T> submit (Callable<T> callable) { FutureTask<T> future = new FutureTask(callable); execute(future); return future; } 复制代码`
...
![](https://user-gold-cdn.xitu.io/2019/6/6/16b2a49180ab5bfc?imageView2/0/w/1280/h/960/ignore-error/1)
...
![](https://user-gold-cdn.xitu.io/2019/6/6/16b2a49194a5f10a?imageView2/0/w/1280/h/960/ignore-error/1)
...
![](https://user-gold-cdn.xitu.io/2019/6/6/16b2a4919c47a319?imageView2/0/w/1280/h/960/ignore-error/1)
...
![](https://user-gold-cdn.xitu.io/2019/6/6/16b2a4919feb7b6e?imageView2/0/w/1280/h/960/ignore-error/1)

通过现象来看其实哪种都不是，线程既没有继续运行同时线程池也没有退出，会一直卡在这里。

当我们 ` dump` 线程快照会发现：

![](https://user-gold-cdn.xitu.io/2019/6/6/16b2a491cdcea71c?imageView2/0/w/1280/h/960/ignore-error/1)

这时线程池中还有一个线程在运行，通过线程名称会发现这是新创建的一个线程（之前是 ` Thread-0` ，现在是 ` Thread-1` ）。

它的线程状态为 ` WAITING` ，通过堆栈发现是卡在了 ` CustomThreadPool.java:272` 处。

![](https://user-gold-cdn.xitu.io/2019/6/6/16b2a491a00ca117?imageView2/0/w/1280/h/960/ignore-error/1)
...
* [一个线程罢工的诡异事件]( https://link.juejin.im?target=https%3A%2F%2Fcrossoverjie.top%2F2019%2F03%2F12%2Ftroubleshoot%2Fthread-gone%2F )
...
![](https://user-gold-cdn.xitu.io/2019/6/6/16b2a491e9556da1?imageView2/0/w/1280/h/960/ignore-error/1)
...
![](https://user-gold-cdn.xitu.io/2019/6/6/16b2a491d0d5b3e0?imageView2/0/w/1280/h/960/ignore-error/1)